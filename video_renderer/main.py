...
def select_music_directory(base: Path) -> Path:
    ...
    console.print()
    print_info(
        f"{len(music_dirs)} müzik klasörü bulundu. Hangisini kullanmak istersiniz?"
    )
    console.print()
    ...
    for i, name in enumerate(dir_names, 1):
        count_audio = sum(
            1
            for f in music_dirs[name].iterdir()
...
def run_post_render_review_cli(
    out_path: Path, target_seconds: int, target_specs: dict
) -> None:
    ...
    def show_summary() -> None:
        ...
        console.print(
            f"  Durum: {'[success]Uygun[/]' if result.valid else '[error]Sorunlu[/]'}"
        )
        console.print(
    ...
    while True:
        c = ask_choice(
            "Kontrol secenegi", ["Devam et", "Detay goster", "Ozeti tekrar goster"], 1
        )
        if c == 1:
            break
        if c == 2:
                ...
                try:
                    if mode == "single" and single_video_path:
                        if single_video_path.exists():
                            single_video_path.rename(
                                archive_dir / single_video_path.name
                            )
                        print_success(
                            f"Kaynak video arsivlendi: {archive_dir.as_posix()}"
                        )
                    else:
                        ...
                        print_success(
                            f"Kaynak intro/loop arsivlendi: {archive_dir.as_posix()}"
                        )
                except Exception as e:
    ...
    try:
        ...
        if music_mode == 2:  # Sorted
            ...
            selected_indices = ask_multiple_choice(
                "Muzikleri secin", [t.name for t in tracks], min_count=1
            )
        ...
        keep_source_audio = ask_confirm(
            "Kaynak videonun orijinal sesi korunsun mu?", False
        )
        ...
        console.print(
            f"  [bold]Müzik Modu:[/] {['Rastgele', 'Sirali', 'Manuel', 'Yok'][music_mode - 1]}"
        )
        console.print(f"  [bold]Arka plan:[/] {len(chosen_bgs)} dosya")
        console.print(
            f"  [bold]Kaynak Ses:[/] {'Evet' if keep_source_audio else 'Hayir'}"
        )
        console.print()
        ...
        audio_fade_in = 5.0
        audio_fade_out = 15.0
        apply_fades = ask_confirm(
            f"Fade efekti uygulansin mi? (In: {audio_fade_in}s, Out: {audio_fade_out}s)",
            False,
        )
        ...
        for i, pair in enumerate(pairs, 1):
            ...
            try:
                ...
                def encode_video():
                    nonlocal video_only
                    encoder.normalize_video(
                        pair.intro, intro_norm, None, keep_audio=keep_source_audio
                    )
                    encoder.normalize_video(
                        pair.loop, loop_norm, None, keep_audio=keep_source_audio
                    )
                    video_only = encoder.concat_videos(
                        intro_norm,
                        loop_norm,
                ...
                def process_audio():
                    ...
                    if job_tracks:
                        music_loop = audio_processor.create_music_loop(
                            job_tracks, total_seconds, global_music_db
                        )
                        if chosen_bgs:
                            bg_processed = audio_processor.process_backgrounds(
                                chosen_bgs
                            )
                            audio_full = audio_processor.mix_tracks(
                        ...
                        if bg_processed:
                            ...
                            if len(bg_processed) > 1:
                                audio_full = audio_processor.mix_tracks(
                                    audio_full, bg_processed[1:], total_seconds
                                )
                    else:
                        audio_full = None
                ...
                print_success(
                    f"[{i}/{len(pairs)}] {pair.name} tamamlandı: {out_path.name}"
                )
                ...
                try:
                    meta = {
                        ...
                        "music_tracks": [
                            t.name
                            for t in (job_tracks if "job_tracks" in locals() else [])
                        ],
                    ...
                    if post_action == "archive":
                        ...
                        job_archive = (
                            archive_dir / f"{time.strftime('%Y%m%d')}_{pair.name}"
                        )
                        ...
                        if pair.intro.exists():
                            shutil.move(
                                str(pair.intro), str(job_archive / pair.intro.name)
                            )
                        if pair.loop.exists():
                            shutil.move(
                                str(pair.loop), str(job_archive / pair.loop.name)
                            )

                        # Save meta
                        (job_archive / "render_info.json").write_text(
                    ...
                    (log_archive / f"meta_{out_path.stem}.json").write_text(
                        json.dumps(meta, indent=2)
                    )
                ...
                for f in tmp_dir.glob("*"):
                    if (
                        f.is_file()
                        and f != out_path
                        and f.suffix.lower() not in [".txt", ".log"]
                    ):  # Keep logs
                        try:
                            f.unlink()
                        except:
        ...
        console.print(
            f"[bold]Toplam süre:[/] {int(total_time // 60)}m {int(total_time % 60)}s"
        )
...
def select_render_mode(videos: List[Tuple[Path, VideoInfo]]) -> str:
    ...
    """
    mode_idx = ask_choice(
        "Render modu", ["Intro + Loop", "Tek Video (sesi degistir)"], 1
    )
...
def configure_render_settings(
    ...
    if config_mode_idx == 1:  # Basit
        ...
        if mode == "single" and single_video_path:
            try:
                ...
                target_fps = (
                    float(ref_info.fps.split("/")[0])
                    / float(ref_info.fps.split("/")[1])
                    if "/" in ref_info.fps
                ...
                print_warning(
                    f"Video analiz hatasi: {e}. Varsayilan 1080p60 kullaniliyor."
                )
            ...
            try:
                ...
                i_fps = (
                    float(i_info.fps.split("/")[0]) / float(i_info.fps.split("/")[1])
                    if "/" in i_info.fps
                    else float(i_info.fps)
                )
                l_fps = (
                    float(l_info.fps.split("/")[0]) / float(l_info.fps.split("/")[1])
                    if "/" in l_info.fps
                    else float(l_info.fps)
                )
                ...
                if i_info.width == l_info.width and i_info.height == l_info.height:
                    ...
                    print_info(
                        "Intro ve Loop cozunurlukleri farkli. 1080p standardi uygulaniyor."
                    )
                ...
                if abs(i_fps - l_fps) < 0.1:  # Neredeyse ayni ise (ornek 29.97 ve 30.0)
                    target_fps = i_fps
                    print_success(
                        f"Intro/Loop FPS eslesiyor: {target_fps:.2f}fps. Kaynak FPS kullanilacak."
                    )
                else:
        ...
        if config_mode_idx == 2:  # Orta
            ...
            if config_mode_idx == 4:  # Ozel
                ...
                if ask_confirm(
                    "Varsayilan FFmpeg parametrelerini duzenlemek ister misiniz?"
                ):
                    print_info(
                        "Not: Bu ozellik su anki surumde komut satirina yansitilacaktir."
                    )
                    codec_config.preset = ask_text(
                        f"Preset ({codec_config.preset})", codec_config.preset
                    )
                    crf_val = ask_int(
                        f"CRF/CQ ({codec_config.crf})", 0, 51, codec_config.crf
                    )
    ...
    print_info(
        f"Ayarlar: {target_width}x{target_height} @ {target_fps} fps | {scale_algo} | Bitrate: {video_bitrate or 'Auto'}"
    )
...
def standardize_audio_files(
    ...
    console.print()
    if not ask_confirm(
        "Muzik dosyalarini otomatik normalize edip arsivlemek ister misiniz?",
        default=True,
    ):
    ...
    if bg_files:
        new_bgs = audio_processor.standardize_tracks(
            bg_files, archive_dir, std_progress
        )
...
def configure_drive_upload() -> Tuple[bool, str]:
    ...
    if ask_confirm(
        "Render bitince videoyu Google Drive'a yedeklemek ister misiniz?", default=False
    ):
        drive_enabled = True
        ...
        try:
            ...
            if not uploader.authenticate():
                print_warning(
                    "Drive girisi yapilamadi! Tarayicida dogrulama gerekebilir."
                )
                print_info(
                    "Lutfen cikan linki takip edin veya credentials.json'i kontrol edin."
                )
        except ImportError:
...
def validate_audio_tracks(
    chosen_tracks: List[Path], run_log: Path, tmp_dir: Path
) -> List[Path]:
    ...
    if invalid_tracks:
        print_error(
            f"Bozuk muzik dosyalari tespit edildi ({len(invalid_tracks)} adet):"
        )
...
def render_pipeline(
    ...
    def _find_existing(pattern: str) -> Optional[Path]:
        ...
        matches = sorted(
            tmp_dir.glob(pattern), key=lambda p: p.stat().st_size, reverse=True
        )
        for m in matches:
    ...
    progress_ctx = (
        MultiStepProgress(steps) if not suppress_progress else DummyProgress()
    )
    ...
    with progress_ctx as progress:
        ...
        video_only_single = (
            tmp_dir / f"video_only_single_{codec_config.codec_family}.mp4"
        )
        ...
        def apply_audio_extras(base_audio: Path) -> Path:
            ...
            if timed_effects:
                ...
                if effects_track and effects_track.exists():
                    audio_out = audio_processor.mix_tracks(
                        audio_out, [effects_track], total_seconds
                    )
        ...
        if mode == "single" and single_video_path:
            ...
            if existing_single:
                ...
                console.print(
                    f"[bold green]  [RESUME] {existing_single.name} mevcut ({existing_single.stat().st_size / 1024 / 1024:.0f}MB), atlaniyor[/bold green]"
                )
            ...
            existing_audio = _find_existing("audio_mixed.*") or _find_existing(
                "music_loop.*"
            )
            if existing_audio:
                audio_full = existing_audio
                console.print(
                    f"[bold green]  [RESUME] {existing_audio.name} mevcut ({existing_audio.stat().st_size / 1024 / 1024:.0f}MB), atlaniyor[/bold green]"
                )
            ...
            def encode_video_branch():
                ...
                if existing_video:
                    ...
                    console.print(
                        f"[bold green]  [RESUME] video_only.mp4 mevcut ({existing_video.stat().st_size / 1024 / 1024:.0f}MB), atlaniyor[/bold green]"
                    )
                    progress.complete_step(0)
                    progress.complete_step(1)
                    progress.complete_step(2)
                ...
                if intro_norm.exists() and intro_norm.stat().st_size > 1000:
                    console.print(
                        f"[bold green]  [RESUME] {intro_norm.name} mevcut, atlaniyor[/bold green]"
                    )
                    intro_time = 0
                    progress.complete_step(0)
                else:
                ...
                if loop_norm.exists() and loop_norm.stat().st_size > 1000:
                    console.print(
                        f"[bold green]  [RESUME] {loop_norm.name} mevcut, atlaniyor[/bold green]"
                    )
                    loop_time = 0
                    progress.complete_step(1)
                else:
    ...
    try:
        ...
        result = validator.validate_output(
            ...
            target_specs={
                "codec": "h264"
                if "h264" in codec_config.encoder.lower()
                else ("h265" if "hevc" in codec_config.encoder.lower() else "av1"),
                "width": target_width,
                "height": target_height,
                "fps": target_fps,
                "has_audio": True,
        ...
        if not result.valid and result.errors:
            ...
            if duration_errors:
                ...
                percent_diff = (
                    abs(actual_duration - total_seconds) / total_seconds * 100
                )

                print(f"⚠ Duration Error: {error.message}")
                print(
                    f"  Expected: {total_seconds}s, Got: {actual_duration:.1f}s ({percent_diff:.1f}% off)"
                )
                print()
            ...
            # Duration OK
            print(
                f"  ✓ Duration: {result.duration_seconds:.1f}s (hedef {total_seconds}s)"
            )
            print(f"  ✓ Codec: {result.video_info.get('codec', '?')}")
            print(
                f"  ✓ Resolution: {result.video_info.get('width')}x{result.video_info.get('height')}"
            )
            print(
                f"  ✓ Audio: Mevcut"
                if result.video_info.get("has_audio")
                else "  ⚠ Audio: Yok"
            )
...
def fix_video_duration(video_path: Path, target_seconds: int, fps: int = 60) -> bool:
    ...
    try:
        ...
        if fixed_duration > 10 and abs(fixed_duration - target_seconds) <= tolerance:
            ...
            print(
                f"  [WARN] MP4 stream copy still wrong ({fixed_duration:.1f}s), trying MKV..."
            )
            tmp_fixed.unlink()
            ...
            try:
                ...
                if (
                    fixed_duration > 10
                    and abs(fixed_duration - target_seconds) <= tolerance
                ):
    ...
    def step_settings_mode(s):
        ...
        s["settings_mode"] = ask_choice(
            "Ayarlar Modu",
            [
                "Otomatik / Native (Her video kendi codec/cozunurlugunu korur)",
                "Tek Tip (Tum videolari ayni formata cevir)",
            ],
            1,
        )  # Raises BN
        return 0
    ...
    def step_global_config(s):
        if s["settings_mode"] == 2:
            s["global_config"] = configure_render_settings(
                "intro_loop", None, None, None
            )  # Raises BN inside likely?
        else:
    ...
    def step_concurrency(s):
        console.print()
        s["max_workers"] = ask_choice(
            "Ayni anda kac video islensin?",
            ["1 (Sirali)", "2 (Es zamanli)", "3 (Es zamanli)"],
            3,
        )  # Raises BN
        return 0
    ...
    def step_audio_strategy(s):
        ...
        # BG Strategy
        bg_strat = ask_choice(
            "Arkaplan sesi (BG) nasil olsun?",
            [
                "Rastgele (Her videoya farkli)",
                "Hicbirinde olmasin",
                "Hepsinde ayni (Sec...)",
            ],
            1,
    ...
    def step_confirm_start(s):
        c = ask_choice(
            f"{len(s['jobs'])} is baslatilsin mi?", ["Evet", "Hayir (Cikis)"], 1
        )  # BN
        if c == 2:
            return 1
        return 0
    ...
    while 0 <= curr < len(steps):
        ...
        try:
            ...
            if curr > 0:
                ...
                if ask_confirm("Cikilsin mi?", False):
                    return 0
...
def run_interactive(ozel1_mode: bool = False) -> int:
    ...
    def step_select_ram_mode(s):
        ...
        if render_cfg.use_ramdisk:
            ...
            if ramdisk:
                ...
                print_warning(
                    "RAM Disk desteklenmiyor veya yer yok, disk kullanilacak."
                )
    ...
    def step_select_mode(s):
        try:
            s["mode"] = select_render_mode(
                s["videos"]
            )  # Has no back logic inside, modify if need
        except BackNavigation:
            raise  # Propagate
        return 0
    ...
    def step_duration_audio(s):
        ...
        if tm == 1:
            ...
            indices = ask_multiple_choice(
                "Track sec", [p.name for p in all_tracks]
            )  # Raises BN
            chosen = [all_tracks[i - 1] for i in indices]

        # Order Selection
        if len(chosen) > 1:
            order_choice = ask_choice(
                "Muzik calma sirasi", ["Karisik (Shuffle)", "Sirali (Secim Sirasi)"], 1
            )
            if order_choice == 1:
        ...
        bg_opts = (
            ["BG kullanma"]
            + ([f"Mevcut BG ({len(all_bgs)})"] if all_bgs else [])
            + ["Track listesinden"]
        )
        ...
        if bg_mode == 1:
            ...
            if sm == 1:
                ...
                idxs = ask_multiple_choice(
                    "BG sec", [p.name for p in all_bgs]
                )  # Raises BN
            ...
            idxs = ask_multiple_choice(
                "BG olacak trackler", [p.name for p in all_tracks], min_count=1
            )  # Raises BN
            for idx in idxs:
                tr = all_tracks[idx - 1]
        ...
        if s.get("ozel1_mode"):
            ...
            if ask_confirm("OZEL1 aktif: Zamanli efekt sesleri eklensin mi?", True):
                effect_pool = sorted(
                    {*all_tracks, *all_bgs}, key=lambda p: p.name.lower()
                )
                if effect_pool:
                    ...
                    for idx in selected:
                        ...
                        interval_sec = ask_int(
                            f"{sfx.name} tekrar araligi (sn)", 1, 86400, 20
                        )
                        max_plays = ask_int(
    ...
    def step_post(s):
        # Output filename
        s["out_path"] = get_output_filename(
            s["mode"], s["single_video_path"], s["codec_family"], s["dur_str"]
        )

        console.print()
        idx = ask_choice(
            "Is bittikten sonra kaynak?", ["Kalsin", "Arsivle", "Sil"], 1
        )  # Raises BN
    ...
    def step_execute(s):
        # Verification
        s["chosen_tracks"] = validate_audio_tracks(
            s["chosen_tracks"], s["run_log"], s["tmp_dir"]
        )
        ...
        sess = {
            ...
            "video": s["single_video_path"].as_posix()
            if s["single_video_path"]
            else None,
        ...
        while 0 <= curr < len(steps):
            ...
            try:
                ...
                if curr > 0:
                    ...
                    if curr == 6:
                        curr = 5  # Back from std_audio -> duration?
                    if curr == 4:
                        curr = 3  # Back from Compat -> Config
                else:
...
Ornekler:
  ...
  python -m video_renderer --tui --rm   # TUI + RAM-optimizasyon modu
    ...
    parser.add_argument(
        "--tui", action="store_true", help="Yeni Textual TUI arayuzunu kullan"
    )
    ...
    if mode != "standard":
        ...
        ramtest_cfg = RamTestConfig(
            enabled=True, use_ramdisk=(mode in ["ramtest", "ramdisk"])
        )
        ...
        try:
            ...
            if session_json.exists():
                try:
                    ...
                    console.print()
                    print_info(
                        f"Onceki session bulundu: {session.get('ts', 'unknown')}"
                    )
