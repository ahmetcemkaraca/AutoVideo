...
def check_nvenc_readiness() -> Dict[str, Any]:
    ...
    Returns:
        Dict with check results:
        {
            ...
            "cuda_version": str,     # CUDA version (from nvidia-smi)
    ...
    try:
        ...
        if smi.returncode == 0 and smi.stdout.strip():
            ...
            if len(parts) >= 4:
                ...
                result["issues"].append("nvidia-smi çıktısı beklenmeyen formatta")
        else:
            result["issues"].append(
                "nvidia-smi çalıştırılamadı veya GPU bulunamadı. "
                "NVIDIA driver kurulu mu? https://www.nvidia.com/drivers"
            )
    except FileNotFoundError:
        result["issues"].append(
            "nvidia-smi bulunamadı. NVIDIA driver kurulu değil. "
            "https://www.nvidia.com/drivers adresinden indirin."
        )
    except subprocess.TimeoutExpired:
        result["issues"].append("nvidia-smi zaman aşımına uğradı. GPU takılı/donmuş olabilir.")
    except Exception as e:
        result["issues"].append(f"nvidia-smi hatası: {e}")

    # 2. Check CUDA version from nvidia-smi
    if result["gpu_found"]:
        try:
                ...
                try:
                    ...
                    if major < 3:
                        result["issues"].append(
                            ...
                            f"NVENC için en az 3.0 gerekli (Kepler veya üstü)."
                        )
    ...
    try:
        ...
        if enc_check.returncode == 0:
            ...
            if not found_any:
                result["issues"].append(
                    "FFmpeg'de NVENC encoder bulunamadı. "
                    "FFmpeg'in NVENC desteği ile derlenmiş olması gerekir. "
                    "Windows: gyan.dev/ffmpeg/builds/ adresinden full build indirin. "
                    "Linux: sudo apt install ffmpeg (veya NVIDIA SDK ile derleyin)."
                )
    except FileNotFoundError:
        result["issues"].append("FFmpeg bulunamadı. Lütfen FFmpeg kurun.")
    except Exception as e:
        result["issues"].append(f"FFmpeg encoder kontrol hatası: {e}")
        ...
        for encoder in ["h264_nvenc", "hevc_nvenc", "av1_nvenc"]:
            try:
                ...
                if not working:
                    ...
                    result["issues"].append(f"{encoder} çalışmıyor: {err_msg}")
            except subprocess.TimeoutExpired:
                result["encoders_working"][encoder] = False
                result["issues"].append(f"{encoder} testi zaman aşımına uğradı")
            except Exception as e:
                result["encoders_working"][encoder] = False
                result["issues"].append(f"{encoder} test hatası: {e}")
