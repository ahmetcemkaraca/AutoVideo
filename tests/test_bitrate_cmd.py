...
class TestBitrateCommand(unittest.TestCase):
    def setUp(self):
        ...
        self.codec_config.to_ffmpeg_args.return_value = [
            "-c:v",
            "libx264",
            "-b:v",
            "0",
        ]  # Default config has -b:v 0
        self.codec_config.encoder = "libx264"

        self.encoder = VideoEncoder(
            runner=self.runner, codec_config=self.codec_config, width=1920, height=1080, fps=30
        )
    ...
    def test_no_bitrate(self):
        ...
        cmd = self.encoder._build_normalize_command(source, output, scale_algo="lanczos")
