...
def print_working_directory(path: Path):
    ...
    console.print(f"[muted]📁 Çalışma dizini:[/] [value]{path.as_posix()}[/]")
    console.print()
...
def print_video_table(videos: List[Tuple[Path, VideoInfo]], title: str = "Video Dosyaları"):
    ...
    table.add_column("Çözünürlük", justify="center", width=12)
    table.add_column("FPS", justify="center", width=8)
    table.add_column("Süre", justify="center", width=10)
...
def print_audio_table(files: List[Path], title: str = "Ses Dosyaları"):
    ...
    table.add_column("Tür", justify="center", width=10)
...
def print_video_info_panel(label: str, path: Path, info: VideoInfo):
    ...
    lines = [
        ...
        f"[muted]Çözünürlük:[/] [value]{info.width}x{info.height}[/]",
        f"[muted]FPS:[/] [value]{info.fps}[/]",
        f"[muted]Pixel Format:[/] [value]{info.pix_fmt}[/]",
        f"[muted]Süre:[/] [value]{format_duration(info.duration)}[/]",
    ]
    ...
    while True:
        try:
            ...
            if value < min_val or value > max_val:
                console.print(f"[error]Aralık: {min_val}-{max_val}[/]")
                continue
            return value
        except ValueError:
            console.print("[error]Geçersiz sayı.[/]")
...
def ask_multiple_choice(
    ...
    count = ask_int(f"Kaç adet seçilecek? ({min_count}-{max_count})", min_count, max_count)
    
    for k in range(count):
        while True:
            idx = ask_int(f"{k+1}/{count} seçim", 1, len(options))
            if idx in used:
                console.print("[warning]Bu zaten seçildi.[/]")
                continue
            used.add(idx)
            selected.append(idx)
            break
...
class MultiStepProgress:
    ...
    def update(self, step_index: int, percent: float, description: Optional[str] = None):
