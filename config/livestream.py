...
# -*- coding: utf-8 -*-
"""
YouTube livestream system configuration.

Extracted and adapted from VideoLivestream/livestream/config.py.
Provides configuration for YouTube livestream automation including video sets,
playlists, and streaming settings.
"""
...
class PlaylistConfig:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "PlaylistConfig":
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "name": self.name,
            "tracks": [
                {"file": t.file, "order": t.order}
                for t in self.tracks
            ],
            "backgrounds": [
                {"file": b.file, "gain_db": b.gain_db}
    ...
    def get_background_files(self, bg_dir: Path) -> List[Tuple[Path, float]]:
        result = []
...
class StreamConfig:
    ...
    def from_dict(cls, data: dict) -> "StreamConfig":
        return cls(
            ...
            video_bitrate=data.get("video_bitrate", "4500k"),
...
class GlobalConfig(BaseConfig):
    ...
    def validate(self) -> List[str]:
        ...
        if self.min_duration_minutes < 1:
            errors.append("min_duration_minutes must be at least 1")

        if self.max_duration_minutes < self.min_duration_minutes:
            errors.append("max_duration_minutes must be >= min_duration_minutes")

        if not self.stream.stream_key:
            errors.append("stream_key is required")
